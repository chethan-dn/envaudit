...
describe('nestjs config service integration', () => {
  it('does not report ENV_UNUSED for variables accessed via ConfigService.get', async () => {
    const result = await createScanOrchestrator((policy) => getBuiltinPlugins(policy)).scan(
      fixturePath('nestjs-config-service'),
    );
